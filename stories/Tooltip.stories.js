...
// ─────────────────────────────────────────────────────────────
// Docs 用コンポーネント説明
// Figmaデザインシステム "Design-System-2.0--AI-Native-" Tooltip ページより
// node-id: 245:11317
// ─────────────────────────────────────────────────────────────
...
export default {
  ...
  argTypes: {
    ...
    id: {
      control: 'text',
      description:
        'role="tooltip" の参照用ID。トリガー要素の aria-describedby に指定して読み上げ対応する',
...
// Default（デフォルト）
// ─────────────────────────────────────────────────────────────
/** デフォルト。短いテキストのツールチップ。Figma「基本設計 / デフォルト」に対応。 */
...
// ─────────────────────────────────────────────────────────────
/**
 * テキストが長い場合。最大幅 320px に達すると折り返す。
 * Figma「基本設計 / テキストが長い場合」に対応。
 */
export const LongText = {
  parameters: {
    docs: {
      description: {
        story:
          'テキストが長い場合。max-width 320px で折り返す。Figma「テキストが長い場合」バリエーション。',
      },
    },
  },
  args: {
...
// ─────────────────────────────────────────────────────────────
/** Figma「基本設計」のデフォルトとテキストが長い場合を並べて比較。 */
...
// ─────────────────────────────────────────────────────────────
// WithHoverDemo（ホバーで表示のデモ）
// ─────────────────────────────────────────────────────────────
/**
 * マウスホバーで表示/非表示する実際の動作デモ。
 * ツールチップはトリガー要素の上（4px 離れた位置）に表示される。
 */
export const WithHoverDemo = {
  parameters: {
    docs: {
      description: {
        story:
          'ホバーで表示/非表示するデモ。`position: absolute` ＋ `bottom: calc(100% + 4px)` で上に配置している（Figma 配置ルール：4px 離す）。',
      },
    },
  },
  render: () => {
    ...
    wrapper.style.cssText =
      'display: inline-flex; flex-direction: column; align-items: flex-start; gap: 40px; padding: 60px 24px 24px;';
    ...
    const topTooltip = createTooltip({ text: '補足説明テキスト', id: 'demo-tooltip-top' });
    topTooltip.style.cssText =
      'position: absolute; bottom: calc(100% + 4px); left: 50%; transform: translateX(-50%); white-space: nowrap; display: none;';
    ...
    topBtn.addEventListener('mouseenter', () => { topTooltip.style.display = 'block'; });
    topBtn.addEventListener('mouseleave', () => { topTooltip.style.display = 'none'; });
    ...
    const bottomTooltip = createTooltip({ text: '補足説明テキスト', id: 'demo-tooltip-bottom' });
    bottomTooltip.style.cssText =
      'position: absolute; top: calc(100% + 4px); left: 50%; transform: translateX(-50%); white-space: nowrap; display: none;';
    ...
    bottomBtn.addEventListener('mouseenter', () => { bottomTooltip.style.display = 'block'; });
    bottomBtn.addEventListener('mouseleave', () => { bottomTooltip.style.display = 'none'; });
...
// ─────────────────────────────────────────────────────────────
// PlacementExamples（Figmaの配置例）
// ─────────────────────────────────────────────────────────────
/**
 * Figma「配置ルール」に示された3種類の配置例。
 * ラベルテキスト / アイコン / 補足ヘルプアイコンへのホバー。
 */
export const PlacementExamples = {
  parameters: {
    docs: {
      description: {
        story:
          'Figma「配置ルール」の配置例3種（ラベルテキストのホバー / アイコンのホバー / 補足ヘルプアイコンのホバー）。各トリガーにマウスオーバーすると確認できる。',
      },
    },
  },
  render: () => {
    ...
    const makeHoverTooltip = (trigger, tooltip, position = 'top') => {
      ...
      if (position === 'top') {
        tooltip.style.cssText =
          'position: absolute; bottom: calc(100% + 4px); left: 50%; transform: translateX(-50%); white-space: nowrap; display: none; z-index: 10;';
      } else {
        tooltip.style.cssText =
          'position: absolute; top: calc(100% + 4px); left: 50%; transform: translateX(-50%); white-space: nowrap; display: none; z-index: 10;';
      }
      trigger.addEventListener('mouseenter', () => { tooltip.style.display = 'block'; });
      trigger.addEventListener('mouseleave', () => { tooltip.style.display = 'none'; });
    ...
    ex1Label.style.cssText =
      "font-family: 'Hiragino Sans', sans-serif; font-size: 12px; color: #515a65;";
    ex1Label.textContent = '例：ラベルテキストのマウスホバー';

    const truncatedText = document.createElement('span');
    truncatedText.style.cssText = [
      "font-family: 'Hiragino Sans', sans-serif",
      'font-weight: 300',
      'font-size: 14px',
      'line-height: 1.5',
    ...
    const ex1Container = makeHoverTooltip(
      truncatedText,
      createTooltip({ text: '【もんじゃ焼】オリジナル京風塩もんじゃ', id: 'tt-label' }),
      'top'
    );
    ...
    iconBtn.style.cssText =
      'display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; padding: 0; background: none; border: none; cursor: pointer;';
    iconBtn.innerHTML = SETTINGS_ICON;
    iconBtn.setAttribute('aria-label', '設定');

    const ex2Container = makeHoverTooltip(
      iconBtn,
      createTooltip({ text: '設定', id: 'tt-icon' }),
      'top'
    );
    ...
    fieldLabel.style.cssText =
      "font-family: 'Hiragino Sans', sans-serif; font-size: 14px; font-weight: 600; color: #222;";
    fieldLabel.textContent = '店舗名';

    const helpIcon = document.createElement('button');
    helpIcon.type = 'button';
    helpIcon.style.cssText =
      'display: inline-flex; align-items: center; justify-content: center; width: 16px; height: 16px; padding: 0; background: none; border: none; cursor: pointer;';
    helpIcon.innerHTML = HELP_ICON;
    helpIcon.setAttribute('aria-label', 'ヘルプ');

    const helpContainer = makeHoverTooltip(
      helpIcon,
      createTooltip({ text: '店舗の名前を入力してください', id: 'tt-help' }),
      'top'
    );
